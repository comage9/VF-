...
class Dashboard {
    constructor(csvUrl, chartId, options = {}) {
        ...
        this.SMOOTHING_CONFIG = {
            enabled: true,              // 평활화 활성화 여부
            historicalWeight: 0.0,      // 과거 패턴 가중치 (0%) - 사용 안 함
            linearWeight: 1.0,          // 선형 분배 가중치 (100%) - 순수 선형
            movingAverageWindow: 3,     // 이동평균 윈도우 크기 (3시간)
            minIncrementPerHour: 10     // 시간당 최소 증가량
        };
    }
    ...
    addPredictiveValues(values) {
        ...
        if (this.aiPredictions && Object.keys(this.aiPredictions).length > 0) {
            ...
            console.warn('⚠️ aiPredictions 없음, 예측 건너뜀');
        }
    ...
    getCurrentActualValue() {
        ...
        // 방법 2: total 값 (최신 누적값)
        const totalValue = parseInt(currentData.total) || 0;

        // 더 큰 값 사용 (더 정확할 가능성)
        const actualValue = Math.max(hourValue, totalValue);
    ...
    getCurrentDayData() {
        ...
        // 데이터에서 오늘 날짜 찾기
        return this.data.find(item => item.date === todayStr) || null;
    }
    ...
    async fetchAIAnalysis() {
        ...
        try {
            ...
            const validSameDayData = sameDayData.filter(d => d.total > 0);
            const averageTotal = validSameDayData.length > 0
                ? Math.round(validSameDayData.reduce((sum, d) => sum + d.total, 0) / validSameDayData.length)
                : 0;
            ...
            const context = {
                ...
                averageTotal: averageTotal, // 평균값 추가
    ...
    calculateBasePredictionsForAI(currentHour, currentData) {
        console.log(`\n🎯 AI 기준값 계산 (가중 평활화): 현재 ${currentHour}시, 값: ${currentData.total}`);

        const predictions = {};
        const currentValue = currentData.total;
    ...
    // 📚 헬퍼: 선형 기준 증가량 계산
    // 📚 헬퍼: 선형 기준 증가량 계산 (Spike 방지: 현재값 무시, 과거 패턴만 사용)
    ...
    calculateTrendMultiplier(currentHour, currentValue, sameDayData) {
        ...
        if (currentHour < 3 || sameDayData.length < 5) {
            ...
            return Math.max(0.8, Math.min(trendMultiplier, 1.2)); // 범위 축소하여 안전하게
        }
        ...
        for (let i = 0; i < lookback; i++) {
            ...
            // 현재 데이터의 구간 증가량
            const currData = { ...this.getCurrentDayData(), total: currentValue }; // 현재값 보정
            // 만약 hour_XX 값이 아직 업데이트 안되었을 수 있으므로 강제 주입
            currData[`hour_${String(currentHour).padStart(2, '0')}`] = currentValue;

            const currInc = this.getHourlyIncrement(currData, targetH);

            // 과거 데이터들의 평균 구간 증가량
            const avgInc = this.getAverageHourlyIncrement(sameDayData, targetH);

            if (avgInc > 10 && currInc > 0) { // 의미 있는 물량일 때만 비율 계산
        ...
        console.log(`📈 추세 분석 (Median): ${ratios.map(r => r.toFixed(2))} ➔ ${medianRatio.toFixed(2)}`);

        // 4. 안전 범위 적용 (0.8 ~ 1.5)
        return Math.max(0.8, Math.min(medianRatio, 1.5));
    }

    getHourlyIncrement(data, hour) {
        if (!data || hour < 1) return 0;
        const currKey = `hour_${String(hour).padStart(2, '0')}`;
        const prevKey = `hour_${String(hour - 1).padStart(2, '0')}`;

        const curr = parseInt(data[currKey]) || 0;
        const prev = parseInt(data[prevKey]) || 0;

        return Math.max(0, curr - prev);
    }

    getAverageHourlyIncrement(dataSet, hour) {
        ...
        dataSet.forEach(row => {
            const inc = this.getHourlyIncrement(row, hour);
            if (inc > 0) {
                sum += inc;
                count++;
            }
            ...
            if (result.success && result.predictions) {
                ...
                if (aiFinalValue) {
                    ...
                    if (recent7DayAvg && recent7DayAvg > 0) {
                        improvedFinalValue = recent7DayAvg;
                        console.log(`✅ 최종값 개선: AI(${aiFinalValue}) → 7일평균(${improvedFinalValue})`);
                        console.log(`   개선율: ${((improvedFinalValue - aiFinalValue) / aiFinalValue * 100).toFixed(1)}%`);
                    } else {
