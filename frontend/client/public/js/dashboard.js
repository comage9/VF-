...
class Dashboard {
    ...
    async fetchAIAnalysis() {
        ...
        let dataCutoffHour = nowHour;
        if (currentData.date === todayStr) {
            dataCutoffHour = Math.max(0, nowHour - 1);
            if (typeof lastNonZeroHour === 'number' && lastNonZeroHour >= 0) {
                dataCutoffHour = Math.min(dataCutoffHour, lastNonZeroHour);
            }
        } else {
            if (typeof lastNonZeroHour === 'number' && lastNonZeroHour >= 0) {
                dataCutoffHour = lastNonZeroHour;
            }
        }
