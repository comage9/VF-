...
| `/master` | 제품 마스터 | 사양·VF·로케이션·출고 리포트 팝업 |
...
| `/product-display` | **제품 배치도** | 총괄(A~E동 미니) + 동별 상세 · A동 세로 슬롯 라인(1~19)·L7 바퀴슬림 2품목/칸 · **B동 6분류 재진열(옷걸이/바지걸이/핸들러/로코스/슬림웨건/리빙카트 137품목, 3품목/칸)** · **C동 와이드 서랍장 73품목(단수+출고순, 2품목/칸)** · 툴팁(분류+제품명+재고+출고 4일치 가중) · 미배치=A/B/C동 배치 통합 제외 · 위치 검색(제품명/로케이션/제품번호/바코드→동·슬롯 이동) · localStorage |
