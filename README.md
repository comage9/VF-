...
| `/product-display` | **제품 배치도** | 총괄(A~E동 미니) + 동별 상세 · **A동 132칸 분류 블록 배치 (rank-a-v26: 엑셀 왕복 — 툴바 '엑셀 다운로드'(뱀모양 번호순, 다품목 동일번호 다행)/'엑셀 업로드'(행순서 순차배치, 누락분 임시보관함), 기본 1칸 1품목·요청 시 2~3품목 가능. 사용자 확정 배치표 — 모던플러스+로코스 42칸 로케이션 1~44(38/43 빈칸), 273 신규 포함, 자가 치유 로직 브라우저 동기화. 구버전: rank-a-v23 (모던 25 loc1~25 + 로코스 21 loc26~46), rank-a-v20 (L1~L2=모던 플러스, L3=슬림서랍장·슬림형 서랍장 전용 블록 분리, L4=로코스·이유 정리함, L5~L6=북/옷걸이/초대형/맥스/탑백, L7=와이드/트레이/데크/해피/소분류, X1/X2=슈즈랙/슬림웨건, 칵투스 제외·📦임시보관함))** · **칸 직접 입력 모드**(수정 모드에서 칸 클릭 → 인라인 input, 콤마 구분 다품목 `19,28`, Enter 저장·Esc 취소, 빠진 제품 → 📦임시보관함 자동 이동, 임시보관함 제품을 칸에 입력하면 자동 제거, 자동 저장) · B동 6분류 3개/칸(137품목) · **C동 108칸 (R20 신규 8칸 와이드 상위 배치)** · D동 탑백·초대형·해피(75품목) · **드래그앤드롭 재배치**(A동 insert 밀림+자리이탈 패널, B/C/D 칸 내부 순서) · **수정 모드**(셀 클릭 선택 + 빈영역 사각형 드래그로 라인/영역 일괄 선택 → 위/아래 그룹 이동 — 칸 위에서는 **Shift+드래그**로 선택, 2026-08-28 · **인쇄 여백 최소화**(배치도 @page 4mm·확대 1080×745) · **C동 행 라벨** → 폐지, **물리 그리드 좌표 체계**(2026-08-28): B/C/D동 렌더 존 기준 동적 행 라벨(왼쪽, "C12"=C동 12행)·열 헤더(상단, "A"~"AA") + 칸 툴팁에 좌표 표시(예: [C11-C]) — "C11-C에 ○○○ 배치"처럼 좌표로 부르면 존 ID로 변환해 배치) · **칸 클릭→목적지 클릭으로 슬롯 좌표 이동**+이동 가능 위치 하이라이트 · **라인 교환**(선택 라인↔목적지 라인 좌표 통째 교환) · **+ 라인 추가**(빈 슬롯 라인 생성) · **+ 칸 추가**(새 빈 칸 생성→드래그 배치) · **칸 드래그 자유 이동**(다른 칸=swap, 빈 공간=자유 좌표) · **여유 ⬆⬇⬅➡ 그리드 확장**(+40px) · **총괄 탭 크로스동 이동**(미니맵에서 A→C 등 다른 동으로 드래그, A동 점유 칸 append 가드) · **📦 임시 보관함**(수정 모드 전용: 칸 선택→보관함에 넣기 / 드래그로 넣고 빼기, 점유 칸=스왑 자동 보관, 별도 키 영속) · **🚫 3개월 미출고 탭**(is_no_outbound_3m 135품목 별도 목록 — 진열 대상 제외) · **4개 탭 동일 형식**(배치/미배치/임시보관함/미출고 = 분류 그룹 → 클릭 시 세부 품목 + 현재고, **제품목록 기준 = 제품 마스터** `/api/master/specs`, **배치 탭 개수 뱃지**) · **동별 화면 패널 따라다님**(renderRightPanel 공통, 동별 배치 필터) · **칸 툴팁 출고 표시**(최근 1개월 N박스/W원 — 배치 기준=박스 수량) · **출고 순 스왑**(같은 분류 내 배치 하위↔미배치 상위 교체, 1개월 출고 실시간 계산 표시) · **총괄 우측 패널 개선**(배치/미배치 분류 아코디언·넓이 560px·자리이탈 품목 클릭→이동 모드·임시 보관함 배치) · **제품 배정 다이얼로그 검색**(번호/이름/분류/바코드/로케이션 검색 → 선택 배정, 기존 제품 자동 자리이탈 이동) · **분류+단수 필터**(모던플러스→5단) · **중복 배치 방지**(한 제품은 전역 1칸만 — 동 우선순위 A→B→C→D sanitize, 로드/복원/저장 전 경로 + 서버 저장 시점 검증, 2026-08-28) · 드래그·이동 즉시 자동 저장 · 조건 기반 재배치 도구: `python backend/scripts/vf_...[truncated]
...
## 10.5. 배치도 매일 재고 체크 (cron, 2026-08-28)
