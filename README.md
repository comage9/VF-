...
| `/product-display` | **제품 배치도** | 총괄(A~E동 미니) + 동별 상세 · **A동 132칸 전체 배치 (1개월 출고 상위 132품목, 칵투스 제외·📦임시보관함, 분류 순: 모던 플러스→슬림→로코스→나머지)** · B동 6분류 3개/칸(137품목) · **C동 108칸 (R20 신규 8칸 와이드 상위 배치)** · D동 탑백·초대형·해피(75품목) · **드래그앤드롭 재배치**(A동 insert 밀림+자리이탈 패널, B/C/D 칸 내부 순서) · **수정 모드**(셀 클릭 선택 + 빈영역 사각형 드래그로 라인/영역 일괄 선택 → 위/아래 그룹 이동 · **칸 클릭→목적지 클릭으로 슬롯 좌표 이동**+이동 가능 위치 하이라이트 · **라인 교환**(선택 라인↔목적지 라인 좌표 통째 교환) · **+ 라인 추가**(빈 슬롯 라인 생성) · **+ 칸 추가**(새 빈 칸 생성→드래그 배치) · **칸 드래그 자유 이동**(다른 칸=swap, 빈 공간=자유 좌표) · **여유 ⬆⬇⬅➡ 그리드 확장**(+40px) · **총괄 탭 크로스동 이동**(미니맵에서 A→C 등 다른 동으로 드래그, A동 점유 칸 append 가드) · **📦 임시 보관함**(수정 모드 전용: 칸 선택→보관함에 넣기 / 드래그로 넣고 빼기, 점유 칸=스왑 자동 보관, 별도 키 영속) · **🚫 3개월 미출고 탭**(is_no_outbound_3m 135품목 별도 목록 — 진열 대상 제외) · **4개 탭 동일 형식**(배치/미배치/임시보관함/미출고 = 분류 그룹 → 클릭 시 세부 품목 + 현재고, **제품목록 기준 = 제품 마스터** `/api/master/specs`) · **출고 순 스왑**(같은 분류 내 배치 하위↔미배치 상위 교체, 1개월 출고 실시간 계산 표시) · **총괄 우측 패널 개선**(배치/미배치 분류 아코디언·넓이 560px·자리이탈 품목 클릭→이동 모드·임시 보관함 배치) · **제품 배정 다이얼로그 검색**(번호/이름/분류/바코드/로케이션 검색 → 선택 배정, 기존 제품 자동 자리이탈 이동) · **분류+단수 필터**(모던플러스→5단) · 드래그·이동 즉시 자동 저장 · 조건 기반 재배치 도구: `python backend/scripts/vf_...[truncated]
